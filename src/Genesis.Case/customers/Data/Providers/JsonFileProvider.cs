...
public abstract class JsonFileProvider<TKey, TEntity> : IJsonFileProvider<TKey, TEntity>
{
    ...
    public async Task<TEntity> CreateAsync(TEntity entity)
    {
        ...
        PropertyInfo? IdExtractor(TEntity x) => x?.GetType().GetProperty("Id");

        var idPropertyInfo = IdExtractor(entity);

        if (idPropertyInfo != null)
        {
            var lastEnt = (await ReadAllAsync()).OrderBy(x => (TKey) IdExtractor(x)!.GetValue(x)!).Last();
            var lastId = idPropertyInfo.GetValue(lastEnt);

            if (lastId is int)
            {
                idPropertyInfo.SetValue(entity, (lastId is int id ? id : 0) + 1);
            }
        }
    ...
    public async Task<TEntity?> ReadAsync(TKey id)
    {
        ...
        var keyEqualsPredicate = new Func<TEntity, bool>(x =>
        {
            ...
            return isContainsId && idPropertyInfo!.GetValue(x)!.Equals(id);
        });
